...
ReferenceColumn.prototype = {
  ...
  get waitFor() {
    if (this._waitFor === undefined) {
      ...
      var res = _processWaitForList(
        wfDef,
        self._baseReference,
        self._currentTable,
        self,
        self._mainTuple,
        'pseudo-column=`' + self.displayname.value + '`',
      );
  ...
  sourceFormatPresentation: function (templateVariables, columnValue, mainTuple) {
    ...
    if (self.display.sourceMarkdownPattern) {
      ...
      Object.assign(keyValues, templateVariables, selfTemplateVariables);
      return processMarkdownPattern(self.display.sourceMarkdownPattern, keyValues, self.table, context, {
  ...
  renderInputIframeUrl: function (data, linkedData) {
    ...
    var url = _renderTemplate(self.inputIframeProps.urlPattern, keyValues, baseTable.schema.catalog, {
...
PseudoColumn.prototype.getAggregatedValue = function (page, contextHeaderParams) {
  ...
  if (self.sourceObject.aggregate.indexOf('array') != -1 && typeof self.sourceObject.array_options === 'object') {
    ...
    if (self.sourceObject.array_options.order) {
      columnOrder = _processColumnOrderList(self.sourceObject.array_options.order, column.table);
    }
  ...
  var getArrayValue = function (val) {
    ...
    var arrayRes = _formatUtils.printArray(val.map(getFormattedValue), {
  ...
  projection =
    ...
    keyColNameEncoded +
    ';v:=' +
    self.sourceObject.aggregate +
    '(' +
    currTable +
    ':' +
    (isRow ? '*' : fixedEncodeURIComponent(column.name)) +
    ')';
...
PseudoColumn.prototype._determineSortable = function () {
  ...
  if (this.isUnique) {
    if (this.isEntityMode) {
      ...
      var display = fk.getDisplay(this._context);
...
Object.defineProperty(ForeignKeyPseudoColumn.prototype, 'display', {
  get: function () {
    if (this._display_cached === undefined) {
      var res = {},
        fkDisplay = this.foreignKey.getDisplay(this._context),
        sourceDisplay = {};
      // attach display defined on the source
      if (this.sourceObject && this.sourceObject.display) {
        var displ = this.sourceObject.display;
...
Object.defineProperty(KeyPseudoColumn.prototype, 'display', {
  get: function () {
    if (this._display_cached === undefined) {
      var res = {},
        keyDisplay = this.key.getDisplay(this._context),
        sourceDisplay = {};
      // attach display defined on the source
      if (this.sourceObject && this.sourceObject.display) {
        var displ = this.sourceObject.display;
...
FacetColumn.prototype = {
  ...
  get sourceReference() {
    if (this._sourceReference === undefined) {
       ...
       */
      if (self.hasPath) {
        newLoc = newLoc.addJoin(self.sourceObjectWrapper, table.schema.name, table.name);
      }
    ...
    if (this._hideNullChoice === undefined) {
      var getHideNull = function (self) {
        ...
        var othersHaveNull =
          ...
          self.reference.facetColumns.some(function (fc, index) {
  ...
  _determineSortable: function () {
    ...
    var rowOrder = _processColumnOrderList(self._facetObject.order, self._column.table, { allowNumOccurrences: true });
  ...
  getChoiceDisplaynames: function (contextHeaderParams) {
    ...
    // otherwise generate an ermrest request to get the displaynames.
      ...
      self.choiceFilters.forEach(function (f, index) {
        ...
        filterStr.push(fixedEncodeURIComponent(columnName) + '=' + fixedEncodeURIComponent(f.term));
      ...
      // create a url
      createRef(filterStr)
        .read(self.choiceFilters.length, contextHeaderParams, true)
        .then(function (page) {
  ...
  entityCounts: function (columnDisplayname, sortColumns, hideNumOccurrences, dontAllowNull) {
    ...
    if (!hideNumOccurrences || sortCounts) {
      ...
      if (self._ref.location.hasJoin) {
        countName =
          'cnt_d(' + self._ref.location.facetBaseTableAlias + ':' + fixedEncodeURIComponent(self._ref.facetBaseTable.shortestKey[0].name) + ')';
      }
