...
function getServer(uri, contextHeaderParams) {
  if (uri === undefined || uri === null) throw new InvalidInputError('URI undefined or null');
...
Catalog.prototype = {
  ...
  currentSnaptime: function (contextHeaderParams) {
    ...
    self._get(contextHeaderParams, true).then(
      function (response) {
  ...
  _fetchSchema: function () {
    ...
    self.server.http
      .get(self._uri + '/schema', { headers: headers })
    ...
    if (this._displayKey === undefined) {
      if (this.keys.length() !== 0) {
        ...
        this._displayKey = keys.sort(function (keyA, keyB) {
          var keyACol = keyA.colset.columns[0],
    ...
    for (var i = 0; i < jsonTable.column_definitions.length; i++) {
      ...
      if (col.type.typename === 'text' && _annotations.ASSET in col.annotations) {
        ...
        if (col.name in assignedColumns) {
          ...
          var annot = col.annotations[_annotations.ASSET],
            valid = true,
            temp = {};
  ...
  _toURI: function (filter, output, sortby, paging, row, limit) {
    ...
    if (output !== undefined && output !== null) {
      for (var c = 0; c < output.length; c++) {
        ...
        if (c === 0) uri = uri + '/' + fixedEncodeURIComponent(col);
        else uri = uri + ',' + fixedEncodeURIComponent(col);
      }
    }
    ...
    if (sortby !== undefined && sortby !== null) {
      for (var d = 0; d < sortby.length; d++) {
        ...
        if (sortby[d].column instanceof Column) {
          // if Column object
          sortCol = sortby[d].column.name;
        }
        var order = sortby[d].order === 'desc' ? '::desc::' : '';
        if (d === 0) uri = uri + '@sort(' + fixedEncodeURIComponent(sortCol) + order;
        else uri = uri + ',' + fixedEncodeURIComponent(sortCol) + order;
      }
      uri = uri + ')';
      ...
      if (paging !== undefined && paging !== null && row !== undefined && row !== null) {
        ...
        for (d = 0; d < sortby.length; d++) {
          ...
          else value = fixedEncodeURIComponent(value);
          if (d === 0) uri = uri + value;
          else uri = uri + ',' + value;
        }
...
export function Column(table, jsonColumn, assetCategoryInfo) {
  ...
  this.formatvalue = function (data, context, options) {
    ...
    var getFormattedValue = function (v) {
      ...
      if (
        (self.type.name.indexOf('int') === 0 || self.type.name.indexOf('serial') === 0) &&
   ...
   */
  this.isSystemColumn =
    _systemColumns.find(function (c) {
...
Column.prototype = {
  ...
  getDisplay: function (context) {
    if (!(context in this._display)) {
      ...
      var columnCommentDisplayMode = _commentDisplayModes.tooltip,
        commentRenderMarkdown;
      if (isObjectAndNotNull(displayProps)) {
        if (_isValidModelCommentDisplay(displayProps.column_comment_display)) {
          columnCommentDisplayMode = displayProps.column_comment_display;
  ...
  getDisplay: function (context) {
    if (!(context in this._display)) {
      ...
      if (typeof showFKLink !== 'boolean') {
        showKeyLink = _getHierarchicalDisplayAnnotationValue(self, context, 'show_key_link');
      ...
      var displayProps = _getHierarchicalDisplayAnnotationValue(this, context, 'comment_display', false);
      var commentDisplay = _commentDisplayModes.tooltip,
        commentRenderMarkdown;
      if (isObjectAndNotNull(displayProps)) {
        if (_isValidModelCommentDisplay(displayProps.column_comment_display)) {
...
InboundForeignKeys.prototype = {
  ...
  _contextualize: function (context) {
    ...
    var self = this,
      ...
      fk,
      i;
    var definitions = this._table.sourceDefinitions,
      wm = _warningMessages;
...
ForeignKeyRef.prototype = {
  ...
  get compressedDataSource() {
    if (this._compressedDataSource === undefined) {
      ...
      this._compressedDataSource = JSON.stringify(ds);
    }
  ...
  getDisplay: function (context) {
    if (!(context in this._display)) {
      var self = this,
        ...
        columnOrder = [],
        showFKLink = true,
        inputDisplayMode = _foreignKeyInputModes[0],
        toTableAnnotation = -1,
      ...
      var fromName,
        ...
        fromComment = null,
        toComment = null,
        fromCommentDisplayMode,
        toCommentDisplayMode,
        commentRenderMarkdown;
      ...
      if (typeof showFKLink !== 'boolean') {
        showFKLink = _getHierarchicalDisplayAnnotationValue(self, context, 'show_foreign_key_link');
