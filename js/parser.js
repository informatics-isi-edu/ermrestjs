 ...
 * service name.
 */
export function parse(uri, catalogObject) {
 ...
 */
export function createSearchPath(catalogId, schemaName, tableName, searchTerm, searchColumNames) {
 ...
 * @return {string}            a path that ERMrestJS understands and can parse, can be undefined
 */
export function createPath(catalogId, schemaName, tableName, facets, cfacets) {
  verify(typeof catalogId === 'string' && catalogId.length > 0, 'catalogId must be a string.');
  verify(typeof tableName === 'string' && tableName.length > 0, 'tableName must be a string.');
 ...
 * @return {string}            a path that ERMrestJS understands and can parse, can be undefined
 */
export function createLocation(service, catalogId, schemaName, tableName, facets, cfacets, catalogObject) {
...
function Location(uri, catalogObject) {
  ...
  const modifiers = uri.split(this._compactUri)[1]; // emtpy string if no modifiers
  ...
  const joinRegExp = /(?:left|right|full|^)\((.*)\)=\((.*:.*:.*)\)/,
    facetsRegExp = /\*::facets::(.+)/,
    customFacetsRegExp = /\*::cfacets::(.+)/;

  let table = this._rootTableName,
    schema = this._rootSchemaName;
  let pathParts = [],
    alias,
    match,
    prevJoin = false,
  ...
  parts.forEach((part, index) => {
    ...
    if (match) {
      if (facets) {
        throw new InvalidFacetOperatorError(this._path, _facetingErrors.duplicateFacets);
      }
      facets = new ParsedFacets(match[1], this._path);
      return;
    }
    ...
    if (match) {
      if (cfacets) {
        throw new InvalidCustomFacetOperatorError(this._path, _facetingErrors.duplicateFacets);
      }
      cfacets = new CustomFacets(match[1], this._path);
      return;
    }

    // filter
    filtersString = part;
    filter = _processFilterPathPart(part, this._path);
  });
...
Location.prototype = {
  ...
  get compactPath() {
    if (this._compactPath === undefined) {
      ...
      uri += this.pathParts.reduce(function (prev, part) {
        var res = prev;
        if (part.joins) {
          res += part.joins.reduce(function (prev, join) {
    ...
    var parsedPartsWithoutJoin = self.pathParts.map(function (part, index) {
      ...
      if (part.customFacets) {
        ...
        if (part.customFacets.facets) {
          facetRes = _renderFacet(
            part.customFacets.facets.decoded,
            part.alias,
            part.schema,
            part.table,
            self.catalog,
    ...
    if (this._sortObject === undefined) {
      if (this._sort !== undefined) {
        ...
        for (var s = 0; s < sorts.length; s++) {
          var sort = sorts[s];
          var column = sort.endsWith('::desc::') ? decodeURIComponent(sort.match(/(.*)::desc::/)[1]) : decodeURIComponent(sort);
   ...
   */
  set sortObject(so) {
    if ((!so && !this._sort) || so === this._sort) return;
 ...
 * @private
 */
const _getQueryParams = function (params) {
  var queryParams = {},
    parts = params.split('&'),
    part,
    i;
 ...
 * @private
 */
export function _getSortModifier(sort) {
  // if no sorting
 ...
 * @private
 */
export function _getPagingModifier(values, isBefore) {
  // no paging
  if (!Array.isArray(values) || values.length === 0) {
    return '';
  }
...
export const _convertSearchTermToFilter = function (term, column, alias, catalogObject) {
  ...
  if (term && term !== '') {
    ...
    terms = terms.filter(function (term) {
      return isStringAndNotEmpty(term);
    });
...
function _createParsedJoinFromStr(linking, table, schema) {
  ...
  return new ParsedJoin(
    ...
    {
      fromCols: fromCols.map(function (colName) {
        return decodeURIComponent(colName);
      }),
      fromColsStr: linking[1],
      toCols: toCols.map(function (colName) {
        return decodeURIComponent(colName);
      }),
      toColsStr: linking[2],
    }, // columnMapping
