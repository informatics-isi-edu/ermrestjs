...
Reference.prototype = {
  ...
  get displayname() {
    ...
    if (!this._displayname) this._displayname = this._table.displayname;
  ...
  get _readAttributeGroupPathProps() {
    if (this._readAttributeGroupPathProps_cached === undefined) {
      ...
      this._readAttributeGroupPathProps_cached = this._location.computeERMrestCompactPath(
        allOutBounds.map(function (ao) {
  ...
  _generateFacetColumns: function (skipMappingEntityChoices) {
    ...
    if (skipMappingEntityChoices) {
      ...
      self
        .validateFacetsFilters(andFilters, facetObjectWrappers, searchTerm, skipMappingEntityChoices)
        .then(function (res) {
          self._facetColumns = finalize(res);
  ...
  validateFacetsFilters: function (facetAndFilters, facetObjectWrappers, searchTerm, skipMappingEntityChoices, changeLocation) {
    var defer = new DeferredPromise(),
      ...
      checkedObjects = {},
      j,
      facetLen = facetObjectWrappers.length,
      andFilterObject;
    /*
    ...
    facetAndFilters.forEach(function (facetAndFilter) {
    ...
    try {
      ...
      this._server.http
        ...
        .then(function (response) {
          ...
          var hasPrevious,
            hasNext = false;
          if (!ownReference._location.paging) {
            // first page
            hasPrevious = false;
            hasNext = response.data.length > limit;
          } else if (ownReference._location.beforeObject) {
            // has @before()
          ...
          if (
            dontCorrectPage !== true &&
            !ownReference.location.afterObject &&
            ownReference.location.beforeObject &&
            (page.tuples.length < limit || !page.hasPrevious)
          ) {
  ...
  update: function (tuples, contextHeaderParams) {
    ...
    try {
      ...
      var self = this,
        ...
        newAlias = '_n',
        uri =
          this._location.service +
          '/catalog/' +
          this.table.schema.catalog.id +
          '/attributegroup/' +
          urlEncode(this.table.schema.name) +
          ':' +
          urlEncode(this.table.name) +
          '/';
        ...
        if (column.isPseudo) {
          ...
          if (column.isAsset) {
            ...
            for (n = 0; n < keyColumns.length; n++) {
              const keyColumnName = keyColumns[n].name;
      ...
      this._server.http
        .put(uri, submissionData, config)
        .then(function updateReference(response) {
  ...
  delete: function (tuples, contextHeaderParams) {
    ...
    try {
      ...
      if (!useTuples) {
        ...
        this._server.http
          .delete(self.location.ermrestCompactUri, config)
          .then(function () {
            defer.resolve();
          })
  ...
  deleteBatchAssociationTuples: function (parentTuple, tuples, contextHeaderParams) {
    ...
    try {
      ...
      var recursiveDelete = function (index) {
        var currFilter = keyValueRes.filters[index];
        var url = [associationRef.location.service, 'catalog', associationRef.location.catalog, 'entity', compactPath + currFilter.path].join('/');
  ...
  get display() {
    if (this._display === undefined) {
      ...
      // if facetpanel won't be used or the clientConfig.facetPanelDisplay doesn't include the current context or a parent context, set to null
      var fpo = null,
        maxFacetDepth = null;
      ...
      if (this._context && ConfigService.verifyClientConfig(true)) {
        // clientConfig.facetPanelDisplay will be defined from configuration or based on chaise defaults
  ...
  getExportTemplates: function (useDefault) {
    ...
    if (!Array.isArray(templates)) {
      var canUseDefault = useDefault && self._context === _contexts.DETAILED && self.defaultExportTemplate != null;

      return canUseDefault ? [self.defaultExportTemplate] : [];
    }

    var finalRes = helpers.replaceFragments(templates, helpers.getExportFragmentObject(self.table, self.defaultExportTemplate));
  ...
  get defaultExportTemplate() {
    if (this._defaultExportTemplate === undefined) {
      ...
      if (self.table.annotations.contains(_annotations.VISIBLE_FOREIGN_KEYS)) {
        var exportRelated = _getRecursiveAnnotationValue(
          _contexts.EXPORT,
          self.table.annotations.get(_annotations.VISIBLE_FOREIGN_KEYS).content,
          true,
        );
      ...
      var exportRefForRelated = hasRelatedExport
        ? self.contextualize.export
        : self._context === _contexts.DETAILED
          ? self
          : self.contextualize.detailed;
  ...
  getAggregates: function (aggregateList, contextHeaderParams) {
    ...
    Promise.all(aggregatePromises)
      .then(
        function getAggregates(response) {
  ...
  generateColumnsList: function (tuple, columnsList, dontChangeReference, skipLog) {
    ...
    if (columns !== -1 && Array.isArray(columns)) {
      for (i = 0; i < columns.length; i++) {
        ...
        // column
          ...
          ignore =
            logCol(typeof col != 'object' || col === null, wm.INVALID_COLUMN, i) ||
    ...
    // heuristics
      ...
      if (systemColumnsMode == true || Array.isArray(systemColumnsMode)) {
        ...
        var addedSystemColumnNames = _systemColumns,
          addedSystemColumns = [];
  ...
  generateActiveList: function (tuple) {
    ...
    var columns = [],
      ...
      requests = [],
      selfLinks = [];
    var consideredUniqueFiltered = {},
      consideredSets = {},
      consideredOutbounds = {},
      consideredAggregates = {},
      consideredSelfLinks = {};
  ...
  get googleDatasetMetadata() {
    if (this._googleDatasetMetadata === undefined) {
      ...
      if (!table.annotations.contains(_annotations.GOOGLE_DATASET_METADATA)) {
        ...
        var metadataAnnotation = _getRecursiveAnnotationValue(
          this._context,
          this._table.annotations.get(_annotations.GOOGLE_DATASET_METADATA).content,
        );
  ...
  _generateRelatedReference: function (fkr, tuple, checkForAssociation, sourceObjectWrapper) {
    ...
    if (checkForAssociation && fkrTable.isPureBinaryAssociation) {
      ...
      var otherFK,
        pureBinaryFKs = fkrTable.pureBinaryForeignKeys,
        assocTable;
  ...
  _getReadPath: function (useEntity, getTRS, getTCRS, getUnlinkTRS) {
    ...
    if (typeof sortObject !== 'undefined') {
      ...
      var hasKey = this._table.keys.all().some(function (key) {
        ...
        return (
          key._notNull &&
          key.colset.columns.every(function (c) {
      ...
      if (!hasKey) {
        for (i = 0; i < this._shortestKey.length; i++) {
          // all the key columns
      ...
      // no sort provieded: use shortest key for sort
      sortObject = [];
    ...
    if (isAttributeGroup) {
      ...
      if (hasSort) {
        sortCols = _modifiedSortObject.map(function (sort) {
          return sort.column;
        });
      } else {
        sortCols = sortObject.map(function (sort) {
          return sort.column;
        });
      }
  ...
  _contextualize: function (context) {
    ...
    if (newTable !== source._table) {
      ...
      if (source._location.hasJoin) {
        ...
        newLocationString =
          source._location.service +
          '/catalog/' +
          catalog.id +
          '/' +
          source._location.api +
          '/' +
        ...
        if (source._location.filter === undefined) {
          ...
          if (filter.type === FILTER_TYPES.BINARYPREDICATE && filter.operator === '=' && sharedKey.colset.length() === 1) {
            ...
            if (
              (source._table._isAlternativeTable() && filter.column === source._table._altForeignKey.colset.columns[0].name) ||
      ...
      if (!newLocationString) {
        ...
        var join;
        if (source._table._isAlternativeTable() && newTable._isAlternativeTable()) {
          join = source._table._altForeignKey.toString(true) + '/' + newTable._altForeignKey.toString();
        } else if (!source._table._isAlternativeTable()) {
          // base to alternative
...
export function Page(reference, etag, data, hasPrevious, hasNext, extraData) {
  ...
  var hasLinkedData = allOutBounds.length > 0 || reference.canUseTCRS || reference.canUseTRS || (associatonRef && associatonRef.canUseTRS);

  if (hasLinkedData) {
    var fks = reference._table.foreignKeys.all(),
      i,
      j,
      d,
      key,
...
Page.prototype = {
  ...
  getContent: function (templateVariables) {
    ...
    if (typeof display.sourceMarkdownPattern === 'string') {
      ...
      pattern = _renderTemplate(display.sourceMarkdownPattern, keyValues, ref.table.schema.catalog, { templateEngine: display.sourceTemplateEngine });
    ...
    if (typeof display._pageMarkdownPattern === 'string') {
      var $page = {
        rows: self.tuples.map(function (t) {
          return t.templateVariables;
        }),
        // (deprecated) should eventually be removed
        values: self.tuples.map(function (t) {
      ...
      pattern = _renderTemplate(ref.display._pageMarkdownPattern, { $page: $page }, ref.table.schema.catalog, {
        templateEngine: ref.display.templateEngine,
      });
...
Tuple.prototype = {
  ...
  get reference() {
    if (this._ref === undefined) {
      ...
      var uri = this._pageRef._location.service + '/catalog/' + this._pageRef._location.catalog + '/' + this._pageRef._location.api + '/';
    ...
    if (this._canUpdate === undefined) {
      ...
      // check row level permission
        ...
        if (this._pageRef._context === _contexts.EDIT) {
          ...
          if (ref.table == self._pageRef.table) {
            // make sure not alternative
            canUpdateOneCol = ref.columns.some(function (col) {
              return (
                !col.inputDisabled &&
                !col._baseCols.some(function (bcol) {
                  return !self.checkPermissions(_ERMrestACLs.COLUMN_UPDATE, bcol.name);
                })
              );
            });
          }
  ...
  get values() {
    if (this._values === undefined) {
      ...
      var checkUpdateColPermission = function (col) {
        return !self.checkPermissions(_ERMrestACLs.COLUMN_UPDATE, col.name);
      };
  ...
  get templateVariables() {
    if (this._templateVariables === undefined) {
      ...
      var keyValues = _getRowTemplateVariables(self._pageRef._table, context, self._data, self._linkedData);
...
Citation.prototype = {
  ...
  compute: function (tuple, templateVariables) {
    var table = this._table,
      citationAnno = this._citationAnnotation;
  ...
  Object.keys(metadataAnnotation).forEach(function (key) {
    if (typeof metadataAnnotation[key] == 'object' && metadataAnnotation[key] != null && !Array.isArray(metadataAnnotation[key])) {
      ...
      metadataAnnotation[key].forEach(function (element) {
        metadata[key].push(_renderTemplate(element, templateVariables, table.schema.catalog, { templateEngine: templateEngine, allowObject: true }));
      });
    } else {
