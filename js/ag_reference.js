...
AttributeGroupReference.prototype = {
  ...
  get uri() {
    if (this._uri === undefined) {
      ...
      this._uri = [loc.service, 'catalog', loc.catalog.id, 'attributegroup', this.ermrestPath].join('/');
    }
    return this._uri;
  },
  ...
  get unfilteredReference() {
    ...
    var newLocation = new AttributeGroupLocation(
      this.location.service,
      this.location.catalog,
      [fixedEncodeURIComponent(this.table.schema.name), fixedEncodeURIComponent(this.table.name)].join(':'),
    );
    return new AttributeGroupReference(this._keyColumns, this._aggregateColumns, newLocation, this._catalog, this.table, this._context);
  },
  ...
  get ermrestPath() {
    if (this._ermrestPath === undefined) {
      ...
      if (
        (loc.afterObject && loc.afterObject.length !== self.ermrestSortObject.length) ||
      ...
      self.ermrestSortObject.forEach(function (so) {
        ...
        keyColumns.push(fixedEncodeURIComponent(so.column) + ':=' + fixedEncodeURIComponent(so.term));
      });
      ...
      this._server.http
        ...
        .then(function (response) {
          ...
          var hasPrevious,
            hasNext = false;
          if (hasPaging) {
            if (!currRef.location.paging) {
              // first page
              hasPrevious = false;
              hasNext = response.data.length > limit;
            } else if (currRef.location.beforeObject) {
              // has @before()
          ...
          if (response.data.length > limit) {
            ...
            if (!currRef.location.beforeObject)
              response.data.splice(response.data.length - 1); // @before, remove first row
            else response.data.splice(0, 1);
          }
          ...
          if (dontCorrectPage !== true && currRef.location.beforeObject && (response.data.length < limit || !hasPrevious)) {
            ...
            var referenceWithoutPaging = new AttributeGroupReference(
              currRef._keyColumns,
              currRef._aggregateColumns,
              newLocation,
              currRef._catalog,
              currRef.table,
              currRef._context,
            ...
            referenceWithoutPaging.read(limit, contextHeaderParams).then(
              function rereadReference(rereadPage) {
...
AttributeGroupPage.prototype = {
  ...
  _getSiblingReference: function (next) {
    ...
    return new AttributeGroupReference(
      currRef._keyColumns,
      currRef._aggregateColumns,
      newLocation,
      currRef._catalog,
  ...
  get uniqueId() {
    if (this._uniqueId === undefined) {
      ...
      this._uniqueId = self._page.reference.shortestKey.reduce(function (res, c, index) {
        hasNull = hasNull || !isDefinedAndNotNull(data[c.name]);
...
export function BucketAttributeGroupReference(baseColumn, baseRef, min, max, numberOfBuckets, bucketWidth) {
  ...
  var binTerm =
    'bin(' +
    fixedEncodeURIComponent(baseColumn.name) +
    ';' +
    numberOfBuckets +
    ';' +
    fixedEncodeURIComponent(min) +
    ';' +
    fixedEncodeURIComponent(max) +
    ')';

  var keyColumns = [new AttributeGroupColumn('c1', binTerm, baseColumn, baseColumn.displayname, baseColumn.type, baseColumn.comment, true, true)];
  ...
  var aggregateColumns = [new AttributeGroupColumn('c2', countName, null, 'Number of Occurrences', new Type({ typename: 'int' }), null, true, true)];
...
BucketAttributeGroupReference.prototype.sort = function () {
  verify(false, 'Invalid function');
};

BucketAttributeGroupReference.prototype.search = function () {
  verify(false, 'Invalid function');
};
