...
import { jsonldSchemaPropObj } from '@isrd-isi-edu/ermrestjs/js/utils/json_ld_schema';
 ...
 */
export default function validateJSONLD(jsonLdOrig) {
  ...
  try {
    if (jsonldSchemaPropObj) {
      ...
      $log.error('Validation property data json not defined!! \n');
    }
  } catch (err) {
    $log.error(err);
  }
  ...
  if (schemaTypeObj) {
    Object.keys(obj).forEach(function (key) {
      if (!key.startsWith('@')) {
        ...
        if (!propDetails) {
          propDetails = getPropDetailsFromParent(key, schemaTypeObj.parent, jsonldSchemaPropObj);
        ...
        if (Array.isArray(obj[key])) {
          obj[key].forEach(function (element, index) {
            if (!isValidType(propDetails, element, key)) {
              $log.warn('Deleting invalid element inside array for ' + key + '\n');
...
function removeProp(obj, key) {
  $log.warn('Invalid attribute ignored ' + key + ' inside type ' + obj[typeKeyword] + ' in JSON-LD\n');
  delete obj[key];
}
...
function isJsonLdBaseValid(obj) {
  var definitionObj = jsonldSchemaPropObj;
  var result = true;
  Object.keys(obj).forEach(function (key) {
    if (key.startsWith('@')) {
      switch (key) {
        case contextKeyword:
          ...
          if (contextURL.host != contextForSchemaOrg && contextURL.pathname == '/') {
        ...
        default:
          $log.warn('JSON-LD property ignored in ermrestjs - ' + key);
          removeProp(obj, key);
      }
    }
  });
  ...
  if (!obj[contextKeyword]) {
    ...
    $log.warn(contextKeyword + ' set as ' + obj[contextKeyword]);
  }

  if (!obj[typeKeyword]) {
    obj[typeKeyword] = datasetType;
    $log.warn(typeKeyword + ' set as ' + obj[typeKeyword]);
  }
...
function areRequiredPropsDefined(jsonLd) {
  ...
  var result = true;
  var requiredPropArr =
    (_jsonldSchemaPropObj$jsonLd = jsonldSchemaPropObj[jsonLd[typeKeyword]]) === null || _jsonldSchemaPropObj$jsonLd === void 0
      ? void 0
  ...
  Object.keys(jsonLd).forEach(function (key) {
    if (Object.prototype.hasOwnProperty.call(jsonLd[key], typeKeyword)) {
...
function getPropDetailsFromParent(key, currentType) {
  ...
  return getPropDetailsFromParent(key, jsonldSchemaPropObj[currentType].parent, jsonldSchemaPropObj);
}
