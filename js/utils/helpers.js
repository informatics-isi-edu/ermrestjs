...
export function decodeFacet(blob, path) {
  var err = new InvalidFacetOperatorError(typeof path === 'string' ? path : '', _facetingErrors.invalidString);
 ...
 */
function _underlineToSpace(str) {
 ...
 */
function _replaceDotWithUnderscore(obj) {
  var res = {},
    val,
    k,
    newK;
...
export function _getCandidateRowNameColumn(columnNames) {
  var candidates = ['title', 'name', 'term', 'label', 'accessionid', 'accessionnumber'];

  var removeExtra = function (str) {
    // remove `.`, `-`, `_`, and space
...
export function _generateRowName(table, context, data, linkedData, isTitle) {
  var annotation, template, keyValues, pattern, actualContext;
  ...
  if (pattern == null || pattern.trim() === '') {
    ...
    var candidate = _getCandidateRowNameColumn(Object.keys(data)),
      result;
    if (candidate !== false) {
...
export function _generateKeyPresentation(key, data, context, templateVariables, addLink) {
  ...
  if (display.isMarkdownPattern) {
    unformatted = _renderTemplate(display.markdownPattern, templateVariables, key.table.schema.catalog, {
 ...
 */
function _generateRowURI(table, data, key) {
 ...
 */
function _generateRowLinkProperties(key, data, context) {
  // if data is empty
  if (typeof data === 'undefined' || data === null || Object.keys(data).length === 0) {
    return null;
  }

  var rowname, i, caption, unformatted;
  var table = key.table;
...
export function generateKeyValueFilters(keyColumns, data, catalogObject, pathOffsetLength, displayname) {
  const encode = fixedEncodeURIComponent,
    pathLimit = URL_PATH_LENGTH_LIMIT;
 ...
 */
function _toPrecision(num, precision, minAllowedPrecision) {
...
export const _formatUtils = {
   ...
   */
  printBoolean: function printBoolean(value) {
    if (value === null) {
      return '';
    }
   ...
   */
  printInteger: function printInteger(value) {
    if (value === null) {
      return '';
    }
   ...
   */
  printTimestamp: function printTimestamp(value) {
    if (value === null) {
      return '';
    }
   ...
   */
  printText: function printText(value) {
    if (value === null) {
      return '';
    }
   ...
   */
  printJSON: function printJSON(value) {
...
export function _isValidSortElement(element) {
  return typeof element === 'object' && typeof element.column === 'string' && typeof element.descending === 'boolean';
}
...
export function _isSameHost(url) {
  ...
  if (!isObjectAndNotNull(ConfigService.clientConfig) || ConfigService.clientConfig.internalHosts.length == 0) return null;
  ...
  return ConfigService.clientConfig.internalHosts.some(function (host) {
...
export function _escapeMarkdownCharacters(text) {
  return _escapeReplacementsForMarkdown.reduce(function (text, replacement) {
...
export function _addErmrestVarsToTemplate(obj, catalog) {
  ...
  if (ConfigService.session) {
    ...
    if (
      ...
      session.client.extensions.ras_dbgap_permissions &&
      Array.isArray(session.client.extensions.ras_dbgap_permissions)
    ) {
      var map = {};
...
export function _validateMustacheTemplate(template, keyValues, catalog, ignoredColumns) {
  ...
  if (!conditionalRegex.exec(template)) {
    ...
    var placeholders = template.match(/\{\{([^\{\}]+)\}\}/gi);
...
export function _getTemplateEngine(engine) {
  ...
  if (isObjectAndNotNull(_clientConfig) && isObjectAndNotNull(_clientConfig.templating) && isValid(_clientConfig.templating.engine)) {
...
export function versionDecodeBase32(version) {
  ...
  var s = version.toUpperCase().replaceAll('-', '').replaceAll('O', '0').replaceAll('I', '1').replaceAll('L', '1');
