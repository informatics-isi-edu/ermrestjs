...
export const _renderFacetHelpers = {
  hasNullChoice: function (term) {
    ...
    return (
      Array.isArray(term[choice]) &&
      term[choice].some(function (v) {
  ...
  parseChoices: function (choices, column, catalogObject) {
    ...
    var encode = fixedEncodeURIComponent,
      nullFilter = _ERMrestFilterPredicates.NULL;
  ...
  parseSearchBox: function (search, rootTable, alias, pathPrefixAliasMapping, catalogObject) {
    ...
    var searchColumns = [{ name: '*' }],
      path = '',
      searchDef,
      usedAlias = '',
      proposedAlias = '',
      addAliasPerPath = false,
      addedAliasIndex = 1;
    ...
    return (
      ...
      searchColumns
        ...
        .join(';') +
      (!addAliasPerPath && path.length > 0 ? '/$' + alias : '')
    );
  },
 ...
 * @ignore
 */
export const _renderFacet = function (
  json,
  alias,
  schemaName,
  tableName,
  ...
  for (i = 0; i < and.length; i++) {
    ...
    if (typeof term.sourcekey === 'string') {
      ...
      if (!rootTable) {
        return _renderFacetHelpers.getErrorOutput(
          "Couldn't parse the url since Location doesn't have acccess to the catalog object or main table is invalid.",
          i,
        );
      }
    ...
    if (
      !hasNullChoice &&
      isObjectAndNotNull(referenceObject) &&
      referenceObject.table.aggressiveFacetLookup &&
      referenceObject.facetColumns.length > 0
    ) {
      try {
    ...
    if (isObjectAndNotNull(mappedFacet) && isObjectAndNotNull(mappedFacet.fastFilterSourceObjectWrapper)) {
      ...
      temp = _renderFacetHelpers.parseDataSource(
        term.source,
        sourcekey,
        alias,
        rootTable,
    ...
    if (useRightJoin) {
      rightJoins.push([rightJoinSchemaTable, constraints.join(';'), path].join('/'));
    } else {
  ...
  waitFors.forEach(function (wf, index) {
    ...
    if (wf in currentTable.sourceDefinitions.sources) {
      ...
      var pc = _createPseudoColumn(
         ...
         */
        sd.clone({ sourcekey: wf }, currentTable),
        mainTuple,
      );
...
export const _facetColumnHelpers = {
  ...
  refColToFacetObject: function (refCol) {
    ...
    if (refCol.isForeignKey) {
      ...
      return {
        source: [{ outbound: constraint }, refCol.table.shortestKey[0].name],
    ...
    // integer and serial key columns should show choice picker
    if (refCol._baseCols[0].isUniqueNotNull && (refCol.type.name.indexOf('int') === 0 || refCol.type.name.indexOf('serial') === 0)) {
      obj.ux_mode = _facetFilterTypes.CHOICE;
    }
  ...
  getEntityChoiceRows: function (andFilterObject, contextHeaderParams) {
    ...
    var sendRequest = function () {
      ...
      ref
        .read(req.keyData.length, contextHeaderParams, true)
        .then(function (page) {
  ...
  runAllEntityChoicePromises: function (promises, resolveCB, rejectCB) {
    ...
    var _runNextPromise = function () {
      ...
      if (obj.mapEntityChoices) {
        ...
        // run the promise
        helpers
          .getEntityChoiceRows(obj.andFilterObject)
          .then(function (res) {
            addToResponse(res, i);
            busySlots--;
            _runNextPromise();
...
SourceObjectWrapper.prototype = {
  ...
  _process: function (table, isFacet, sources) {
    ...
    var hasPath = false,
      ...
      isFiltered = false,
      filterProps = {},
      isAllOutboundNotNull = false,
      isAllOutboundNotNullPerModel = false;
    var lastForeignKeyNode,
      firstForeignKeyNode,
  ...
  toString: function (reverse, isLeft, outAlias, isReverseRightJoin) {
    return this.sourceObjectNodes.reduce((prev, sn, i) => {
      if (sn.isFilter) {
        if (reverse) {
      ...
      var addAlias = outAlias && ((reverse && sn === this.firstForeignKeyNode) || (!reverse && sn === this.lastForeignKeyNode));
  ...
  getRawSourcePath: function (reverse, outAlias) {
    ...
    while (isLast(i)) {
      ...
      if (sn.isPathPrefix) {
        ...
        path = path.concat(
          sn.nodeObject.getRawSourcePath(reverse, self.foreignKeyPathLength == sn.nodeObject.foreignKeyPathLength ? outAlias : null),
        );
      } else if (sn.isFilter) {
...
export const _sourceColumnHelpers = {
  processDataSourcePath: function (source, rootTable, tableName, catalogId, sources) {
    ...
    var hasAnyOfFKProps = function (obj) {
      return [
        srcProps.INBOUND,
        srcProps.OUTBOUND,
        srcProps.REMOTE_SCHEMA,
        srcProps.REMOTE_TABLE,
        srcProps.REMOTE_COLUMNS,
        srcProps.LOCAL_TO_REMOTE_COLUMNS,
        srcProps.LOCAL_COLUMNS,
      ].some(function (p) {
        return p in obj;
      });
    };
    ...
    var i,
      ...
      isAllOutboundNotNull = true,
      isAllOutboundNotNullPerModel = true;
   ...
   */
  parseSourceNodesWithAliasMapping: function (
    sourceNodes,
    lastForeignKeyNode,
    foreignKeyPathLength,
    sourcekey,
   ...
   * @param {*} outAlias
   */
  parseAllOutBoundNodes: function (
    sourceNodes,
    lastForeignKeyNode,
    foreignKeyPathLength,
    sourcekey,
  ...
  parseSourceObjectNodeFilter: function (nodeObject, table) {
    ...
    if ('filter' in nodeObject) {
      ...
      if (('operand_pattern' in nodeObject && operator == nullOperator) || (!('operand_pattern' in nodeObject) && operator != nullOperator)) {
      ...
      if ('operand_pattern' in nodeObject) {
        operand = _renderTemplate(nodeObject.operand_pattern, {}, table.schema.catalog, { templateEngine: nodeObject.template_engine });
  ...
  _sortFilterInSource: function (a, b) {
    const srcProps = _sourceProperties,
      helpers = _sourceColumnHelpers;

    const aHasNegate = srcProps.NEGATE in a,
  ...
  generateForeignKeyName: function (fk, isInbound) {
    ...
    if (eTable.isPureBinaryAssociation) {
      var otherFK,
        pureBinaryFKs = eTable.pureBinaryForeignKeys;
...
SourceObjectNode.prototype = {
  ...
  toString: function (reverse, isLeft, outAlias, isReverseRightJoin) {
    if (this.isForeignKey) {
      var rev = (reverse && this.isInbound) || (!reverse && !this.isInbound);
