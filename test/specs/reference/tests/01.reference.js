...
exports.execute = function (options) {
  describe('For determining reference objects and its child objects, ', function () {
    ...
    var entityWithSlash = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableNameWithSlash;

    var entityWithCompositeKey = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableNameWithCompKey;

    var entityWithDisabledColumns = options.url + '/catalog/' + catalog_id + '/entity/' + schemaNamePermission + ':' + tableWithDisabledColumns;
    ...
    describe('for a single entity,', function () {
      ...
      it('resolve should return a Reference object that is defined.', function (done) {
        options.ermRest
          .resolve(singleEntityUri, { cid: 'test' })
    ...
    describe('for multiple entities with limit less than the number of referenced entities,', function () {
      ...
      it('resolve should return a Reference object that is defined.', function (done) {
        options.ermRest
          .resolve(multipleEntityUri + '/@sort(id)', { cid: 'test' })
          .then(function (response) {
            reference = response;
    ...
    describe('for multiple entities with limit greater than the number of referenced entities,', function () {
      ...
      it('resolve should return a Reference object that is defined.', function (done) {
        options.ermRest
          .resolve(multipleEntityUri + '@sort(value)', { cid: 'test' })
          .then(function (response) {
            reference = response;
    ...
    if (!process.env.CI) {
      describe('for tables with slash(`/`) in their name,', function () {
        ...
        it('resolve should return a Reference object that is defined.', function (done) {
          options.ermRest
            .resolve(entityWithSlash, { cid: 'test' })
            .then(function (response) {
              reference = response;
      ...
      describe('regarding default values, ', function () {
        it('should not use the default value if value is passed in at least one of the rows.', function (done) {
          testCreate(
            ...
            [
              { id: '9900', name: 'New 0' },
              { id: '9901', name: 'New 1', value: 2 },
            ],
            [
              { id: '9900', name: 'New 0', value: null },
              { id: '9901', name: 'New 1', value: 2 },
            ],
            done,
          );
        ...
        it('should use the default value if the value is missing in all the rows.', function (done) {
          testCreate(
            ...
            [
              { id: '9902', name: 'New 2', value: 1234 },
              { id: '9903', name: 'New 3', value: 1234 },
            ],
            done,
          );
        });
        ...
        describe('passing values for disabled columns (acl, or system columns), ', function () {
          ...
          beforeAll(function (done) {
            ...
            utils.setCatalogAcls(
              ...
              entityWithDisabledColumns + '@sort(col_id)',
              catalog_id,
              {
                catalog: {
                  id: catalog_id,
