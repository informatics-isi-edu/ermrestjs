exports.execute = function (options) {
  describe('For alternative tables,', function () {
    ...
    var base1Uri = options.url + '/catalog/' + catalog_id + '/entity/' + schemaNameEncoded + ':' + baseTable1Encoded;

    var base2Uri = options.url + '/catalog/' + catalog_id + '/entity/' + schemaNameEncoded + ':' + baseTable2Encoded + '/value::gt::15@sort(id)';
    ...
    describe('1. for app linking without providing appLinkFn, ', function () {
      ...
      it('1.1 uncontextualized reference should return error, ', function (done) {
        options.ermRest.appLinkFn(null);
        options.ermRest.resolve(base1Uri, { cid: 'test' }).then(
          function (response) {
            reference = response;

            expect(function () {
              appLink = reference.appLink;
            }).toThrow('`appLinkFn` function is not defined.');
            done();
          },
          function (err) {
            console.dir(err);
    ...
    describe('3. for app linking without table annotation,', function () {
      ...
      it('3.1 uncontextualized reference should have default link, ', function (done) {
        options.ermRest.appLinkFn(appLinkFn);
        options.ermRest.resolve(base2Uri, { cid: 'test' }).then(
          function (response) {
            reference = response;
