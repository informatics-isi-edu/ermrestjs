...
exports.execute = function (options) {
  ...
  describe('for testing aggregate functions on tables and columns,', function () {
    ...
    var baseUri = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableName;

    var attrGroupUri = options.url + '/catalog/' + catalog_id + '/attributegroup/M:=' + schemaName + ':' + tableName;

    var compositeTableWithJoinUri =
      options.url +
      '/catalog/' +
      catalog_id +
      '/entity/' +
    ...
    var tableWithJoinUri =
      ...
      schemaName +
      ':' +
      tableNameWithSimpleKey +
      ':simple_id)';

    var tableWithJoinAttrGroupUri =
      options.url +
      '/catalog/' +
      catalog_id +
      '/attributegroup/T:=' +
    ...
    var tableWithUnicodeAttrGroupUri =
      options.url +
      '/catalog/' +
      catalog_id +
      '/attributegroup/T:=' +
    ...
    var histogramAnnotationUri = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableNameHistogramAnnotation;

    beforeAll(function (done) {
      options.ermRest
        .resolve(baseUri, { cid: 'test' })
    ...
    describe('regarding aggregate APIs, ', function () {
      it('if adding even one aggregate column would make the url to go over limit, should return error', function (done) {
        var filter = [],
          col_name = 'column_that_has_a_very_long_name_and_long_values_to_test_limit';
        for (var i = 0; i < 45; i++) {
          filter.push(col_name + '=00000000000000000000000000000000000000000000000000' + i);
        }
        var baseUriLengthy = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableName + '/' + filter.join(';');

        options.ermRest.resolve(baseUriLengthy, { cid: 'test' }).then(function (ref) {
          ref
            .getAggregates([ref.aggregate.countAgg])
            .then(function (response) {
      ...
      it('should get the aggregate count for the aggregate_table table.', function (done) {
        var aggregateList = [reference.aggregate.countAgg];

        reference
          .getAggregates(aggregateList)
          .then(function (response) {
      ...
      it('should get aggregates for int_agg column.', function (done) {
        ...
        var aggregateList = [intColumnAggs.countNotNullAgg, intColumnAggs.countDistinctAgg, intColumnAggs.minAgg, intColumnAggs.maxAgg];
      ...
      it('should get aggregates for float_agg column.', function (done) {
        ...
        var aggregateList = [floatColumnAggs.countNotNullAgg, floatColumnAggs.countDistinctAgg, floatColumnAggs.minAgg, floatColumnAggs.maxAgg];
      ...
      it('should get aggregates for text_agg column.', function (done) {
        ...
        var aggregateList = [textColumnAggs.countNotNullAgg, textColumnAggs.countDistinctAgg, textColumnAggs.minAgg, textColumnAggs.maxAgg];
      ...
      it('should get aggregates for date_agg column.', function (done) {
        ...
        var aggregateList = [dateColumnAggs.countNotNullAgg, dateColumnAggs.countDistinctAgg, dateColumnAggs.minAgg, dateColumnAggs.maxAgg];
    ...
    describe('regarding group aggregate APIs, ', function () {
      ...
      describe('entityCounts, ', function () {
        var expectAttrGroupRef = function (ag_ref, uri, colDisplaynames) {
          ...
          expect(
            ag_ref.columns.map(function (c) {
              return c.displayname.value;
            }),
          ).toEqual(colDisplaynames, 'column names missmatch.');
        };
        ...
        it("if there's a join in the path should return an attributegroup reference, using cnt_d(T:shortestKey) for count.", function (done) {
          options.ermRest
            .resolve(tableWithJoinUri, { cid: 'test' })
        ...
        it('should be able to handle table and columns with unicode characters.', function (done) {
          options.ermRest
            .resolve(tableWithUnicode, { cid: 'test' })
            .then(function (reference) {
              expectAttrGroupRef(
        ...
        it('for int_agg should return an attributegroup reference, using cnt(*) for count.', function () {
          expectAttrGroupRef(reference.columns[1].groupAggregate.entityCounts(), attrGroupUri + '/0:=int_agg;count:=cnt(*)@sort(count::desc::,0)', [
            'int_agg',
        ...
        it('for float_agg should return an attributegroup reference, using cnt(*) for count.', function () {
          expectAttrGroupRef(reference.columns[2].groupAggregate.entityCounts(), attrGroupUri + '/0:=float_agg;count:=cnt(*)@sort(count::desc::,0)', [
            'float_agg',
            'Number of Occurrences',
          ]);
        });
        ...
        it('for text_agg should return an attributegroup reference, using cnt(*) for count.', function () {
          expectAttrGroupRef(reference.columns[3].groupAggregate.entityCounts(), attrGroupUri + '/0:=text_agg;count:=cnt(*)@sort(count::desc::,0)', [
            'text_agg',
            'Number of Occurrences',
          ]);
        });
        ...
        it('for date_agg should return an attributegroup reference, using cnt(*) for count.', function () {
          expectAttrGroupRef(reference.columns[4].groupAggregate.entityCounts(), attrGroupUri + '/0:=date_agg;count:=cnt(*)@sort(count::desc::,0)', [
            'date_agg',
            'Number of Occurrences',
          ]);
        });
      ...
      describe('histograms, ', function () {
        ...
        it('should set properties properly based on anotation value.', function (done) {
          options.ermRest
            .resolve(histogramAnnotationUri, { cid: 'test' })
            .then(function (response) {
              ...
              expect(facetColumns[1].histogramBucketCount).toBe(
                50,
                'dif_num_buckets_float column bucket count is not the same as what is defined in the annotation',
              );
              // default_date column
        ...
        describe('for an integer column,', function () {
          ...
          it('the histogram function should return a proper bucket AG reference.', function () {
            ...
            expect(bucketAgRef instanceof options.ermRest.BucketAttributeGroupReference).toBeTruthy(
              'Reference returned by histogram is not an instance of BucketAttributeGroupReference',
            );
