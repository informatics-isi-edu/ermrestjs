...
exports.execute = function (options) {
  describe('Regarding column list heuristics and logics, ', function () {
    ...
    var singleEnitityUri = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableName + '/id=' + entityId;

    var singleEnitityUriCompositeKey =
      options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableWithCompositeKey + '/id=' + entityId;

    var singleEnitityUriCompositeKey2 =
      options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableWithCompositeKey2 + '/id=' + entityId;

    var singleEnitityUriCompositeKey3 =
      options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableWithCompositeKey3 + '/id=' + entityId;

    var singleEnitityUriSimpleKeyFK =
      options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableWithSimpleKeyFK + '/id=' + entityId;

    var singleEnitityUriWithSlash = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableWithSlash + '/id=' + entityId;

    var singleEnitityUriWithAsset = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableWithAsset + '/id=' + entityId;

    var tableWithInvalidUrlPatternURI = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableWithInvalidUrlPattern;

    var singleEnitityUriSystemColumnsHeuristics =
      options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableWithNoVisibleColumns + '/id=' + entityId;
    ...
    var entryRefExpectedPartialValue = ['1', '9000', '', '4000', '4000', '12', '4000 : 4001', '4000:4003', '4000 : 4002', '4001 : 4002', ''];
    ...
    var entryCreateRefExpectedLinkedValue = ['Hank', '', '4000 : 4002', '1'];

    var entryCreateRefExpectedPartialValue = ['9000', '', '4000 : 4002', '1'];

    var assetEntryExpectedValue = ['1', '1', '1000', '10001', null, 'https://example.org', 'https://example.org', 'https://example.org', 4];
    ...
    beforeAll(function (done) {
      ...
      compactRefExpectedLinkedValue = [
        '<a href="https://example.org/chaise/record/columns_schema:columns_table/id=1">1</a>',
        '<a href="https://example.org/chaise/record/columns_schema:table_w_simple_key/RID=' +
          utils.findEntityRID(options, schemaName, 'table_w_simple_key', 'id', '9000') +
          '">Hank</a>',
        '',
        '<a href="https://example.org/chaise/record/columns_schema:table_w_simple_key/RID=' +
          utils.findEntityRID(options, schemaName, 'table_w_simple_key', 'id', '4000') +
          '">John</a>',
        '<a href="https://example.org/chaise/record/columns_schema:table_w_simple_key_2/RID=' +
          utils.findEntityRID(options, schemaName, 'table_w_simple_key_2', 'id', '4000') +
          '">Hank</a>',
        '4001',
        '4002',
        '4003',
        '',
        '<p><a href="http://example.com" class="external-link-icon external-link">12</a></p>\n',
        '<a href="https://example.org/chaise/record/columns_schema:table_w_composite_key/RID=' +
          utils.findEntityRID(options, schemaName, 'table_w_composite_key', 'id', '1') +
          '">4000 , 4001</a>',
        '<a href="https://example.org/chaise/search">1</a>',
        '<a href="https://example.org/chaise/record/columns_schema:table_w_composite_key/RID=' +
          utils.findEntityRID(options, schemaName, 'table_w_composite_key', 'id', '2') +
          '">4000 , 4002</a>',
        '<a href="https://example.org/chaise/record/columns_schema:table_w_composite_key/RID=' +
          utils.findEntityRID(options, schemaName, 'table_w_composite_key', 'id', '4') +
          '">4001 , 4002</a>',
        '',
      ];
      ...
      assetCompactExpectedValue = [
        ...
        '<a href="https://example.org/chaise/record/columns_schema:columns_table/RID=' +
          utils.findEntityRID(options, schemaName, 'columns_table', 'id', '1') +
          '">1</a>',
        '1000',
        '10001',
        '<a href="https://example.org?uinit=1&amp;cid=test" download="" class="asset-permission">filename</a>',
      ...
      tableWSlashData = [
        ...
        '2',
        '<a href="https://example.org/chaise/record/columns_schema:table_w_simple_key/RID=' +
          utils.findEntityRID(options, schemaName, 'table_w_simple_key', 'id', '9001') +
          '">Harold</a>',
        '<a href="https://example.org/chaise/record/columns_schema:table_w_simple_key/RID=' +
          utils.findEntityRID(options, schemaName, 'table_w_simple_key', 'id', '9000') +
          '">Hank</a>',
      ...
      describe('when visible-columns annotation is present for the context, ', function () {
            ...
            checkReferenceColumns([
              {
                ...
                expected: [
                  ...
                  'col_6',
                  'col_7',
                  ['columns_schema', 'outbound_fk_5'].join('_'),
        ...
        describe('for asset columns,', function () {
              ...
              checkReferenceColumns([
                {
                  ...
                  expected: ['col_asset_3', 'col_filename', 'col_byte', 'col_md5', 'col_sha256', 'col_asset_6_byte_count'],
                },
              ]);
            });
          });
          ...
          describe('for checking table which has asset column without upload feature, ', function () {
            ...
            var checkCol = function (name) {
              var colIndex = assetRef1.columns.findIndex(function (c) {
                return c.name === name;
              });
            ...
            beforeAll(function (done) {
              options.ermRest.resolve(tableWithInvalidUrlPatternURI, { cid: 'test' }).then(
                function (response) {
                  assetRef1 = response;
        ...
        describe('for inbound foreignkey columns, ', function () {
          ...
          it('in detailed context, should create a pseudo-column for those and remove them from related references.', function () {
            expect(detailedRef.columns[3]._constraintName).toBe(
              ['columns_schema', 'inbound_related_to_columns_table_2_fkey'].join('_'),
              "didn't create a pseudo column.",
            );

            expect(detailedRef.related.length).toBe(1, "didn't remove the column from related references");
            expect(detailedRef.related[0].table.name).toBe(
              'inbound_related_to_columns_table',
              'the name of related reference is not what was expected',
            );
          });
        });
      ...
      describe('when visible-columns annotation is not present for the context, ', function () {
        describe('PseudoColumn for key, ', function () {
          ...
          xit('if key is simple and its constituent columns are part of simple foreign key, should not be added (instead it should apply the PseudoColumn for foreignkey logic.)', function (done) {
            options.ermRest.resolve(singleEnitityUriSimpleKeyFK, { cid: 'test' }).then(
              function (ref) {
          ...
          it('if context is detailed, entry, entry/create, or entry/edit, should not be added.', function (done) {
            ...
            options.ermRest.resolve(singleEnitityUriCompositeKey2, { cid: 'test' }).then(
              function (ref) {
          ...
          describe('otherwise, ', function () {
            ...
            xit('if table has several keys with same size, should pick the one with most text columns.', function (done) {
              options.ermRest.resolve(singleEnitityUriCompositeKey3, { cid: 'test' }).then(
                function (ref) {
                  expect(ref.columns[0].isPseudo).toBe(true);
        ...
        it('should not include serial columns that are part of a simple key, and that key has not been used for self-link.', function (done) {
          options.ermRest.resolve(singleEnitityUriCompositeKey2, { cid: 'test' }).then(
            function (ref) {
        ...
        describe('for columns that are part of composite FKR, ', function () {
          ...
          it('in edit or create context should not include the columns, and just create PseudoColumn for them.', function () {
            var expectedCols = [
              ...
              ['columns_schema', 'outbound_fk_2'].join('_'),
              ['columns_schema', 'outbound_fk_3'].join('_'),
              ['columns_schema', 'outbound_fk_4'].join('_'),
              'columns_schema_outbound_fk_7',
              ['columns_schema', 'outbound_fk_5'].join('_'),
              ['columns_schema', 'outbound_fk_6'].join('_'),
              ['columns_schema', 'outbound_fk_8'].join('_'),
              ['columns_schema', 'outbound_fk_7'].join('_'),
              ['columns_schema', 'outbound_fk_9'].join('_'),
            ];
    ...
    describe('.generateColumnsList, ', function () {
      ...
      describe('with system columns heuristic config options defined,', function () {
        ...
        var compactSystemColumnsModeRef,
          compactSystemColumnsModeColumns,
          detailedSystemColumnsModeRef,
          detailedSystemColumnsModeColumns,
          entrySystemColumnsModeRef,
          entrySystemColumnsModeColumns;
        ...
        beforeAll(function (done) {
          ...
          options.ermRest
            .resolve(singleEnitityUriSystemColumnsHeuristics, {
              cid: 'test',
            })
            .then(function (response) {
    ...
    describe('tuple.values, ', function () {
      ...
      describe('for tables with asset column, ', function () {
        ...
        it('otherwise should return the download button.', function (done) {
          assetRefCompact.read(limit).then(
            function (page) {
              var tuples = page.tuples;
        ...
        it('in detailed, should return the download button and image preview if applicaple.', function (done) {
          assetRefDetailed.read(limit).then(
            function (page) {
              var tuples = page.tuples;
    ...
    function checkReferenceColumns(tesCases) {
      tesCases.forEach(function (test) {
        expect(
          test.ref.columns.map(function (col) {
