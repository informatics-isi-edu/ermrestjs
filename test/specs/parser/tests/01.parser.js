...
exports.execute = function (options) {
  ...
  describe('Filters,', function () {
    describe('single filter,', function () {
      var parsedFilter;
      var entityId = 10;
      var singleFilterPath = '/catalog/' + catalogId + '/entity/' + schemaName + ':' + tableName + '/id=' + entityId;
    ...
    describe('single filter with empty value, ', function () {
      var parsedFilter;
      var singleFilterPath = '/catalog/' + catalogId + '/entity/' + schemaName + ':' + tableName + '/id=';
    ...
    describe('multiple filters with a conjunction,', function () {
      ...
      it('parse should take a uri with multiple filters and return a location object.', function () {
        ...
        expect(location.compactUri).toBe(
          options.url + '/catalog/' + catalogId + '/entity/' + schemaName + ':' + tableName + '/id::gt::' + lowerLimit + '&id::lt::' + upperLimit,
        );
        expect(location.api).toBe('entity');
        expect(location.path).toBe(
          schemaName + ':' + tableName + '/id::gt::' + lowerLimit + '&id::lt::' + upperLimit + '@sort(' + sort + ')' + '@after(' + text + ')',
        );
        expect(location.compactPath).toBe(schemaName + ':' + tableName + '/id::gt::' + lowerLimit + '&id::lt::' + upperLimit);
    ...
    describe('multiple filters with a disjunction,', function () {
      ...
      var multipleFilterPath = '/catalog/' + catalogId + '/entity/' + schemaName + ':' + tableName + '/id=' + firstEntityId + ';id=' + secondEntityId;
      ...
      it('parse should take a uri with multiple filters and return a location object.', function () {
        ...
        expect(location.api).toBe('entity');
        expect(location.path).toBe(schemaName + ':' + tableName + '/id=' + firstEntityId + ';id=' + secondEntityId);
        expect(location.compactPath).toBe(schemaName + ':' + tableName + '/id=' + firstEntityId + ';id=' + secondEntityId);
        expect(location.catalog).toBe(catalogId.toString());
        expect(location.catalogId).toBe(catalogId.toString());
        expect(location.version).toBeNull();
        expect(location.sort).toBeUndefined();
    ...
    describe('quantified value list (any) syntax', function () {
      var parsedFilter;
      var multipleFilterPath = '/catalog/' + catalogId + '/entity/' + schemaName + ':' + tableName + '/some%20col=any(v%201,v%202)';
    ...
    describe('regarding changing filter to facet,', function () {
      ...
      it('should handle single filters.', function () {
        testFilterToFacet('equality', 'col=2', 1, { and: [{ source: 'col', choices: ['2'] }] });

        testFilterToFacet('leq', 'col::leq::12', 1, { and: [{ source: 'col', ranges: [{ max: '12' }] }] });

        testFilterToFacet('geq', 'col::geq::12', 1, { and: [{ source: 'col', ranges: [{ min: '12' }] }] });

        testFilterToFacet('null', 'col::null::', 1, { and: [{ source: 'col', choices: [null] }] });

        testFilterToFacet('search', 'col::ciregexp::test', 1, { and: [{ source: 'col', search: ['test'] }] });
      });

      it('should handle conjunction filter.', function () {
        testFilterToFacet('same column', 'col=1&col::null::', 1, {
          and: [
        ...
        testFilterToFacet('different columns', 'id=1&col::null::', 1, {
          and: [
            { source: 'id', choices: ['1'] },
            { source: 'col', choices: [null] },
          ],
      ...
      it('should handle disjunction filter.', function () {
        testFilterToFacet('same column, same operator', 'col=1;col::null::', 1, { and: [{ source: 'col', choices: ['1', null] }] });

        testFilterToFacet('same column, different operator', 'col=1;col::geq::12', 1, {
        ...
        testFilterToFacet('combination of same and differnt columns', 'col=1;id=2;col::leq::12', 1, {
          or: [
      ...
      it('should handle conjunction of disjunction filters.', function () {
        testFilterToFacet('disjunction of same column, and then conjunction', '(col=1;col::leq::5)&(id=2;id::ciregexp::test)', 1, {
          and: [
        ...
        testFilterToFacet('combination', '(col=1;col2::leq::5;col=5)&(col3=2;col3::ciregexp::test)', 2, {
          and: [
            {
              or: [
      ...
      it('should handle disjunction of conjunction filters.', function () {
        testFilterToFacet('combination', '(col1=1&col2::null::);(col3::ciregexp::123&col4=test)', 2, {
          or: [
            {
              and: [
      ...
      it('should handle quantified value list (any) syntax', function () {
        testFilterToFacet('quantified value list', 'col1=any(val1,val2,val3)&col2=any(test%201,test%202)', 1, {
          and: [
  ...
  describe('Entity linking,', function () {
    ...
    it('parse should take a uri with multiple filters and return a location object.', function () {
      ...
      expect(location.compactPath).toBe(
        schemaName + ':' + tableName + '/id::gt::' + lowerLimit + '&id::lt::' + upperLimit + '/(id)=(' + schemaName + ':' + tableName2 + ':id2)',
      );
      expect(location.sort).toBe('@sort(' + sort + ')');
  ...
  describe('Facets, ', function () {
    ...
    describe('when uri has joins, ', function () {
      it('parser should handle having facet before join.', function () {
        ...
        expect(location.ermrestCompactPath).toEqual(
          'T:=parse_schema:parse_table/accession=1/$T/some-other-column::ciregexp::test/$T/M:=(id)=(s:otherTable:id)',
      ...
      it('parser should handle having facet after join.', function () {
        ...
        expect(location.ermrestCompactPath).toEqual(
          'T:=parse_schema:parse_table/M:=(id)=(s:otherTable:id)/accession=2/$M/some-other-column::ciregexp::test2/$M',
          'ermrestCompactPath missmatch',
        );
      });
      ...
      it('parser should handle having multiple layers of join and facets.', function () {
        uri =
          ...
          '/*::facets::' +
          validBlob +
          '/some_col=v1/(id)=(s:otherTable:id)/(id2)=(s:otherTable2:id)' +
          '/*::facets::' +
          validBlob2 +
          '/some_col2=v2/(id)=(s:yetAnotherTable:id)' +
          '/*::facets::' +
          validBlob3;
        location = options.ermRest.parse(uri);
        expect(location).toBeDefined('location is not defined');
      ...
      it('parser should handle not having any filter on the first layer.', function () {
        uri =
          baseUri +
          `/(id)=(s:table2:id)/!(RID=%3A%3Anull%3A%3A&RCB=%3A%3Anull%3A%3A)/(id3)=(s:table3:id)/*::facets::${validBlob2}/(id4)=(s:table4:id)`;
        location = options.ermRest.parse(uri);
        expect(location).toBeDefined('location is not defined');
        expect(location.uri).toEqual(uri, 'uri missmatch');
        expect(location.ermrestCompactPath).toEqual(
    ...
    describe('regarding handling different facets syntaxes, ', function () {
      var facet, blob;
      var unicodeSample = 'ɐ ɔ',
        encodedUnicodeSample = '%C9%90%20%C9%94';
      ...
      var expectLocation = function (blob, facetObject, path, hasNonSearchBoxVisibleFilters, errMessage, woSchema, woPrefix, aliases) {
        ...
        expect(loc.facets.hasNonSearchBoxVisibleFilters).toBe(
          hasNonSearchBoxVisibleFilters,
          'hasNonSearchBoxVisibleFilters missmatch' + (errMessage ? errMessage : '.'),
        );
        ...
        if (aliases != null) {
          expect(JSON.stringify(loc.pathPrefixAliasMapping.aliases)).toEqual(
            JSON.stringify(aliases),
            'alias mapping missmatch ' + (errMessage ? errMessage : '.'),
          );
        }
      };
      ...
      describe('regarding source attribute, ', function () {
        ...
        it('should accept string as source.', function () {
          expectLocation('N4IghgdgJiBcDaoDOB7ArgJwMYFM4gEYQAaELACxQEtck55CQBdAX1aA', { and: [{ source: '1', choices: ['1'] }] }, '1=1/$M', true);
        });
          ...
          it("when the prefix is used only once and we don't need alias", function () {
            expectLocation(
              'N4IghgdgJiBcDaoDOB7ArgJwMYFM6JFU1wGscBPOEdAFwCN1oBGAfRwhoEsbKBfAGlCcIDNNHwgADmAxIcLJFgAWOALZgQ-amnqMorYaOYsAZiSYgAugJAAlAJIARK1uUpOuJBIvXrQA',
              {
                and: [
                  {
                    source: [{ sourcekey: 'outbound1_entity' }, { inbound: ['parse_schema', 'outbound1_inbound1_fk1'] }, 'RID'],
                    choices: ['1'],
                  },
                ],
              },
            ...
            expectLocation(
              '',
              {
                and: [
                  ...
                  {
                    source: [{ sourcekey: 'outbound1_entity' }, { outbound: ['parse_schema', 'outbound1_fk1'] }, 'RID'],
                    choices: ['2'],
                  },
                ],
              },
        ...
        it('should properly handle array as source', function () {
          expectLocation(
            'N4IghgdgJiBcDaoDOB7ArgJwMYFM6JHQBcAjdafEABzAyRwH0ksALHAWzBABprb6GRMCQA2jAGYBrAIwgAugF9uoAJYQyaCgj51GzNpx6E0pclGkM1G6BamzFvAEoBJACLzerFCtxJK9hUUgA',
            {
              and: [
                {
                  source: [{ outbound: ['parse_schema', 'parse_table_fk1'] }, { inbound: ['parse_schema', 'outbound1_inbound1_fk1'] }, 'RID'],
                  choices: ['1'],
                },
              ],
            },
  ...
  describe('CustomFacets, ', function () {
    var customFacetError = 'Given encoded string for cfacets is not valid.',
      location,
      blob,
      cfacets;
