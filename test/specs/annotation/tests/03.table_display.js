...
exports.execute = function (options) {
  describe('Reference display related APIs', function () {
    ...
    var table1EntityUri = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableName1 + '/@sort(id)';

    var table2EntityUri = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableName2;

    var table3EntityUri = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableName3;

    var table4EntityUri = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableName4;

    var table5EntityUri = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableName5 + '/@sort(id)';

    var table5bEntityUri = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableName5b + '/@sort(id)';

    var table6EntityUri = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableName6 + '/@sort(id)';

    var table7EntityUri = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableName7 + '/@sort(id)';

    var table8EntityUri = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableName8 + '/@sort(id)';

    var table9EntityUri = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableName9 + '/@sort(id)';

    var table10EntityUri = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableName10 + '/@sort(id)';

    var table11EntityUri = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableName11 + '/@sort(id)';

    var tableWoAnnotEntityUri = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableNameWoAnnot;

    var tableCatalogAnnotEntityUri = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableNameCatalogAnnot;

    var tableCompactOptionsEntityUri = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableNameCompactOptions;

    var tableWMaxFacetDepthUri = options.url + '/catalog/' + catalog_id + '/entity/' + schemaName + ':' + tableNameWMaxFacetDepth;
    ...
    describe('table entities without name/title nor table-display:row_name context annotation, ', function () {
      ...
      it('resolve should return a Reference object that is defined.', function (done) {
        options.ermRest.resolve(table1EntityUri, { cid: 'test' }).then(
          function (response) {
            reference = response;
    ...
    describe('table entities with name/title/accession_id without table-display:row_name annotation, ', function () {
      ...
      it('resolve should return a Reference object that is defined.', function (done) {
        options.ermRest.resolve(table2EntityUri, { cid: 'test' }).then(
          function (response) {
            reference = response;

            expect(reference).toEqual(jasmine.any(Object));

            options.ermRest.resolve(table3EntityUri, { cid: 'test' }).then(
              function (response) {
                reference2 = response;
    ...
    describe('table entities without special columns without table-display:row_name annotation, ', function () {
      ...
      it('tuple displayname/rowName should return the display key.', function (done) {
        options.ermRest
          .resolve(tableWoAnnotEntityUri, { cid: 'test' })
    ...
    describe('table entities without name/title nor table-display:row_name context annotation with composite key, ', function () {
      ...
      it('resolve should return a Reference object that is defined.', function (done) {
        options.ermRest.resolve(table4EntityUri, { cid: 'test' }).then(
          function (response) {
            reference = response;
    ...
    describe('table entities with table-display.row-name annotation, without table-display.row-name/unformatted annotation, ', function () {
      ...
      it('resolve should return a Reference object that is defined.', function (done) {
        options.ermRest.resolve(table5EntityUri, { cid: 'test' }).then(
          function (response) {
            reference = response;
    ...
    describe('table entities with table-display.row-name (handlebars) annotation, without table-display.row-name/unformatted annotation, ', function () {
      ...
      it('resolve should return a Reference object that is defined.', function (done) {
        options.ermRest.resolve(table5bEntityUri, { cid: 'test' }).then(
          function (response) {
            reference = response;
    ...
    describe('table entities with table-display.row-name/title annotation.', function () {
      ...
      beforeAll(function (done) {
        options.ermRest
          .resolve(table11EntityUri, { cid: 'test' })
          .then(function (res) {
            ref = res;
            done();
          })
        ...
        ref
          ...
          .then(function (page) {
            ...
            tuples.forEach(function (tuple, i) {
              ...
              expect(tuple.displayname.value).toBe(
                '<span class="new-class">' + expected + '</span>',
                'displayname value missmatch for tuple index=' + i,
              );
              expect(tuple.displayname.unformatted).toBe(
                ':span:' + expected + ':/span:{.new-class}',
                'displayname unformatted missmatch for tuple index=' + i,
              );
            });
            done();
          })
      ...
      it('tuple rowName should NOT use row_name/title', function (done) {
        tuples.forEach(function (tuple, i) {
          expect(tuple.rowName.value).toBe(
            '<strong>' + tuple.values[1] + ' ' + tuple.values[2] + '</strong>',
            'rowName value missmatch for tuple index=' + i,
          );
          expect(tuple.rowName.unformatted).toBe(
            '**' + tuple.values[1] + ' ' + tuple.values[2] + '**',
            'rowName unformatted missmatch for tuple index=' + i,
          );
        });
        done();
      });
    ...
    describe('table entities with table-display.row_markdown_pattern annotation', function () {
      ...
      it('resolve should return a Reference object that is defined.', function (done) {
        options.ermRest.resolve(table7EntityUri, { cid: 'test' }).then(
          function (response) {
      ...
      var markdownPattern =
        ':::iframe [{{{$self.rowName}}}{{#$fkeys.schema_table_display.table_w_t_disp_annot_w_mp_fkey}}(with {{{rowName}}} from catalog {{{$catalog.snapshot}}}){{/$fkeys.schema_table_display.table_w_t_disp_annot_w_mp_fkey}}]({{{$self.uri.detailed}}}) \n:::';
      ...
      it('page.content should return HTML for all tuples using row_markdown_pattern and prefix_markdown and separator_markdown', function () {
        var rowContent = function (id, caption) {
          ...
          return (
            ...
            '<a class="chaise-btn chaise-btn-secondary chaise-btn-iframe" href="' +
            iframeURL +
            '">' +
    ...
    describe('Page.content', function () {
      ...
      describe('when row_markdown_pattern or page_markdown_pattern are not defined for the context, ', function () {
        it('should return an unordered list of clickable row-names.', function (done) {
          var expected =
            '<ul>\n' +
            '<li><a href="https://example.org/chaise/record/schema_table_display:table_wo_title_wo_annotation/RID=' +
            utils.findEntityRID(options, schemaName, tableName1, 'id', '20001') +
            '">20001</a></li>\n' +
            '<li><a href="https://example.org/chaise/record/schema_table_display:table_wo_title_wo_annotation/RID=' +
            utils.findEntityRID(options, schemaName, tableName1, 'id', '20002') +
            '">20002</a></li>\n' +
            '</ul>\n';
        ...
        it('row-names should be using the row_name/<context> context format.', function (done) {
          var expected =
            '<ul>\n' +
            '<li><a href="https://example.org/chaise/record/schema_table_display:table_w_table_display_annotation_w_row_name_context/RID=' +
            utils.findEntityRID(options, schemaName, tableName6, 'id', '10001') +
            '"><strong>Shakespeare</strong></a></li>\n' +
            '<li><a href="https://example.org/chaise/record/schema_table_display:table_w_table_display_annotation_w_row_name_context/RID=' +
            utils.findEntityRID(options, schemaName, tableName6, 'id', '10002') +
            '"><strong>Twain</strong></a></li>\n' +
            '</ul>\n';
    ...
    describe('table entities with $fkeys in their row_markdown_pattern.', function () {
      it('should be able to access row-name and detailed uri of outbound foreign keys in annotation.', function (done) {
        options.ermRest
          .resolve(table8EntityUri, { cid: 'test' })
          .then(function (ref) {
            return ref.read(5);
          })
          .then(function (page) {
            ...
            page.tuples.forEach(function (t, index) {
              if (typeof expected[index] === 'string') {
                ...
                val =
                  '<a href="' +
                  recordURL +
                  '/schema_table_display:table_w_table_display_annotation/RID=' +
                  utils.findEntityRID(options, schemaName, tableName5, 'id', expected[index].id) +
                  '">' +
                  expected[index].rowName +
                  '</a>';
              }
      ...
      it('should be able to access referred table unformatted and formatted data in annotation.', function (done) {
        options.ermRest
          .resolve(table9EntityUri, { cid: 'test' })
          .then(function (ref) {
            return ref.read(5);
          })
          .then(function (page) {
            var expected = ['10001: 10001', '10002: 10002', '10003: 10003', '10004: 10004', '10005: 10005', ''];
            page.tuples.forEach(function (t, index) {
      ...
      it('should be able to access columns with `.` in their names.', function (done) {
        options.ermRest
          .resolve(table10EntityUri, { cid: 'test' })
          .then(function (ref) {
            return ref.read(5);
          })
          .then(function (page) {
            var expected = ['20001: 20001', '20002: 20002', '20003: 20003', '20004: 20004', '20005: 20005', ''];
            page.tuples.forEach(function (t, index) {
              expect(t.displayname.value).toEqual(expected[index], 'index= ' + index + '. displayname missmatch.');
              expect(t.rowName.value).toEqual(expected[index], 'index= ' + index + '. rowName missmatch.');
            });
    ...
    describe('table entities with $catalog in their row_markdown_pattern.', function () {
      it('should be able to access row-name in annotation.', function (done) {
        options.ermRest
          .resolve(tableCatalogAnnotEntityUri, { cid: 'test' })
          .then(function (ref) {
            return ref.read(1);
          })
          .then(function (page) {
    ...
    describe('table with compact options.', function () {
      it('should be able to access options in annotation.', function (done) {
        options.ermRest
          .resolve(tableCompactOptionsEntityUri, { cid: 'test' })
          .then(function (ref) {
    ...
    describe('display.hideTotalCount', function () {
      ...
      it('otherwise should get it from display annotation on schema', function (done) {
        options.ermRest
          .resolve(table1EntityUri, { cid: 'test' })
          .then(function (ref) {
