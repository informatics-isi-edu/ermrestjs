    ...
    describe('Reference related APIs, ', () => {
      it('should ignore the fast filter for tables that disable it.', (done) => {
        const facetObj = {
          and: [
            {
              source: [{ outbound: ['fast_filter_schema', 'table_wo_aggressive_facet_lookup_fk1'] }, 'RID'],
              choices: ['1'],
            },
          ],
        };
        resolveURL('table_wo_aggressive_facet_lookup', facetObj)
          .then((res) => {
      ...
      describe('should honor the given fast filter, ', () => {
        ...
        const testReadAndReadPath = (context) => {
          let ref;
          beforeAll((done) => {
            resolveURL(mainTableName, facetObjWFastFilters)
              .then((response) => {
          ...
          it('read should work properly', (done) => {
            ref
              .read(25)
              .then(function (page) {
