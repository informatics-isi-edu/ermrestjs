...
exports.checkPageValues = function (pageData, tuples, sortBy) {
  ...
  for (var i = 0; i < pageData.length; i++) {
    ...
    for (var j = 0; j < columns.length; j++) {
      ...
      expect(responseData[columnName]).toBe(
        tupleData[columnName],
        'Value ' + responseData[columnName] + ' for column ' + columnName + ' does not match tuple data value',
      );
    }
  }
};
...
exports.setCatalogAnnotations = function (testOptions, annotations) {
  return new Promise((resolve, reject) => {
    ermrestImport
      .setCatalogAnnotations(testOptions.catalogId, annotations)
      .then(() => {
