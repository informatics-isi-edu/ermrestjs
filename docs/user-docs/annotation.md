...
- `{` ... `"display": {` _context_`:` _displayoption_ ...`}` ... `}`: Apply each _displayoption_ to the asset for any number of _context_ names.
- `{`... `"display": ` display_ ... `}`: By doing so, we will show a preview of the selected image in entry contexts to the users. Be mindful that the client will not do any extra checks whether the selected file is an image, and you should guard against it by using `filename_ext_filter` or `filename_ext_regexp`. Setting this flag will also add the image preview to the default asset presentation in `detailed` context (record page).
